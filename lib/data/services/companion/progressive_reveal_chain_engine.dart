...
class ProgressiveRevealChainEngine {
  ...
  Future<ChainRunState> startSession({
    ...
    final resolvedAvg = avgNewMinutesPerAyah == null || avgNewMinutesPerAyah <= 0
    ...
    final updatedVerses = <ChainVerseState>[
      for (final verse in state.verses)
        verse.passedForStage(state.activeStage)
            ...
            : verse
                .markPassedForStage(state.activeStage)
                .copyWith(
                  stage1: verse.stage1.copyWith(
                    weak: state.activeStage == CompanionStage.guidedVisible
                        ? (verse.stage1.weak ||
  ...
  Future<ChainAttemptUpdate> submitCorrectionExposure({
    ...
    if (state.completed || (!stage1Correction && !stage2Correction)) {
    ...
    final nextMode = switch (runtime.phase) {
      ...
      Stage1Phase.spacedConfirmation =>
        updatedVerse.stage1.hasAnyH0Success && !updatedVerse.stage1.spacedConfirmed
  ...
  Future<ChainAttemptUpdate> _submitStage1Attempt({
    ...
    if (runtime.mode == Stage1Mode.correction || currentVerse.stage1.correctionRequired) {
      throw StateError('Correction playback is required before next cold attempt.');
    }
    ...
    if (runtime.mode == Stage1Mode.modelEcho) {
      ...
      final shouldProbe =
          runtime.phase == Stage1Phase.remediation ||
      ...
      final nextMode = shouldProbe
          ? (nextVerse.stage1.hasAnyH0Success && !nextVerse.stage1.spacedConfirmed
              ? Stage1Mode.spacedReprobe
              : Stage1Mode.coldProbe)
    ...
    if (autoCheckRequired &&
      ...
      throw StateError('Auto-check option is required for Stage-1 cold attempts.');
    }
    ...
    if (h0UnassistedPass) {
      ...
      if (lastH0SuccessAtMs != null &&
          runtime.lastActionAtEpochMs - lastH0SuccessAtMs >= runtime.adaptiveSpacingMs) {
    ...
    await _persistAttempt(
      ...
      autoCheckResult: autoCheckRequired ? (autoEval.passed ? 'pass' : 'fail') : null,
    ...
    return ChainAttemptUpdate(
      ...
      telemetry: VerseAttemptTelemetry(
        ...
        autoCheckResult: autoCheckRequired ? (autoEval.passed ? 'pass' : 'fail') : null,
        timeOnVerseMs: updatedVerse.stage1.timeOnVerseMs,
        timeOnChunkMs: afterPass.stage1?.chunkElapsedMs ?? runtime.chunkElapsedMs,
        stage1Mode: attemptMode,
      ),
    );
  }
  ...
  Future<ChainAttemptUpdate> _submitStage2Attempt({
    ...
    if (runtime.mode == Stage2Mode.correction ||
      ...
      throw StateError('Correction playback is required before next Stage-2 attempt.');
    }
    ...
    return ChainAttemptUpdate(
      ...
      telemetry: VerseAttemptTelemetry(
        ...
        autoCheckResult: autoCheckRequired ? (autoEval.passed ? 'pass' : 'fail') : null,
        timeOnVerseMs: updatedVerse.stage2.timeOnVerseMs,
        timeOnChunkMs: advanced.stage2?.chunkElapsedMs ?? runtime.chunkElapsedMs,
        stage2Mode: attemptMode,
  ...
  Future<ChainRunState> _advanceStage2AfterPass({
    ...
    if (runtime.phase == Stage2Phase.checkpoint) {
      ...
      if (nextCursor < targets.length) {
        ...
        return state.copyWith(
          ...
          currentVerseIndex: nextIndex,
          currentHintLevel: _stage2EffectiveBaselineHint(verses[nextIndex].stage2),
          returnVerseIndex: null,
        );
      }
      ...
      final chunkPassRate =
          targets.isEmpty ? 0.0 : (targets.length - failed.length) / targets.length;
      final everyReady = verses.every((verse) => _stage2IsReady(verse, runtime.config));
      ...
      if (runtime.remediationRounds >= runtime.config.maxCheckpointRemediationRounds) {
    ...
    final allReady = verses.every((verse) => _stage2IsReady(verse, runtime.config));
    if (allReady) {
  ...
  Future<ChainRunState> _advanceFromStage2ToStage3({
    ...
    final nextIndex = weakPreludeTargets.isNotEmpty
        ? weakPreludeTargets.first
        : advanced.currentVerseIndex;
    final nextHint = weakPreludeTargets.isNotEmpty
        ? HintLevel.letters
        : _defaultHintForStage(advanced.activeStage);
    return advanced.copyWith(
      verses: verseStates,
      stage2: state.stage2?.copyWith(
        phase: budgetFallback ? Stage2Phase.budgetFallback : Stage2Phase.completed,
        budgetExceeded: budgetFallback || (state.stage2?.budgetExceeded ?? false),
        activeAutoCheckPrompt: null,
      ),
      stage3WeakPreludeTargets: weakPreludeTargets,
      stage3WeakPreludeCursor: 0,
      currentVerseIndex: nextIndex,
      currentHintLevel: nextHint,
      returnVerseIndex: null,
    );
  }
  ...
  ChainRunState _initializeStage2Runtime({
      ...
      for (var i = 0; i < state.verses.length; i++)
        state.verses[i].copyWith(
          stage2: state.verses[i].stage2.copyWith(
            weakTarget:
                state.verses[i].stage2.weakTarget || state.verses[i].stage1.weak,
            cueBaselineHint:
                state.verses[i].stage1.weak ? HintLevel.firstWord : HintLevel.letters,
          ),
        ),
    ];
    ...
    return state.copyWith(
      ...
      currentHintLevel: _stage2EffectiveBaselineHint(verses[currentIndex].stage2),
      returnVerseIndex: null,
    );
  }
  ...
  Future<ChainAttemptUpdate> _submitLegacyAttempt({
    ...
    final updatedVerseState = currentVerseState.copyWith(
      ...
      highestHintLevel: effectiveHintLevel.order >
  ...
  _Stage2Touched _touchStage2Clock({
    ...
    final deltaMs =
        nowMs > runtime.lastActionAtEpochMs ? nowMs - runtime.lastActionAtEpochMs : 0;
    final verses = [...state.verses];
    final currentIndex = state.currentVerseIndex;
    final current = verses[currentIndex];
  ...
  String? _stage2RiskTrigger({
    ...
    if (verse.stage2.consecutiveFailures >= config.discriminationFailureTrigger) {
  ...
  Future<void> _persistAttempt({
    ...
    await _companionRepo.insertVerseAttempt(
      ...
      retrievalStrength: retrievalStrength,
      timeOnVerseMs: verse.stage1.timeOnVerseMs,
      timeOnChunkMs: state.stage1?.chunkElapsedMs ?? 0,
  ...
  Future<void> _upsertProficiency({
    ...
    final passesCount = (existing?.passesCount ?? 0) + (evaluatorPassed ? 1 : 0);
  ...
  _Stage1Target? _pickStage1Target({
    ...
    final spacedEligible = _nextMatchingIndex(
      ...
      predicate: (verse) =>
          ...
          nowEpochMs - verse.stage1.lastH0SuccessAtMs! >= runtime.adaptiveSpacingMs,
    );
